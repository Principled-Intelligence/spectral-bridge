...
async def test_responses_forwards_body_to_target(adapter_client, mock_target):
    ...
    await adapter_client.post("/v1/responses", json=body)

    call_args = mock_target.post.call_args
    assert call_args.args[0] == "http://fake-target.example.com/v1/responses"
    assert call_args.kwargs["json"] == body


async def test_responses_returns_target_response(adapter_client, mock_target):
    target_body = {"output": [{"type": "message", "role": "assistant", "content": []}]}
    mock_target.post.return_value = httpx.Response(201, json=target_body)

    response = await adapter_client.post("/v1/responses", json={"input": "hi"})

    assert response.status_code == 201
    assert response.json() == target_body


async def test_responses_strips_hop_by_hop_headers(adapter_client, mock_target):
    mock_target.post.return_value = httpx.Response(200, json={"output": []})
...
async def test_responses_preserves_other_headers(adapter_client, mock_target):
    mock_target.post.return_value = httpx.Response(200, json={"output": []})
...
async def test_responses_target_unreachable_returns_502(adapter_client, mock_target):
    mock_target.post.side_effect = httpx.ConnectError("connection refused")
...
async def test_responses_target_timeout_returns_502(adapter_client, mock_target):
    mock_target.post.side_effect = httpx.TimeoutException("timed out")
